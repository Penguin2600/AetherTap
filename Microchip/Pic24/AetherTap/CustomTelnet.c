...
// TCPIPConfig.h
	...
	#define MAX_TELNET_CONNECTIONS	(5u)
#endif
...
static ROM BYTE strTitle[]			= "\x1b[2J\x1b[31m\x1b[1m"	// 2J is clear screen, 31m is red, 1m is bold
									  "AetherTap Telnet Server 1.1\x1b[0m\r\n"	// 0m is clear all attributes
								  	  "Login: ";
...
static ROM BYTE strMenu[]			= "====MAIN MENU====\r\n"
									  "[*] Refresh\r\n"
									  "[1] Toggle LED\r\n"
									  "[2-5] Toggle AUX 0-3\r\n"
									  "[R] Reset Device\r\n"
...
static ROM BYTE strLocalIP[]		= "\r\nLocal IP: ";
static ROM BYTE strLEDs[]	    	= "\r\nLEDs: ";
static ROM BYTE strAuxIO[]	    	= "\r\nIO Channels: ";
static ROM BYTE strBreak[]			= "\r\n";
		...
		switch(TelnetState)
		{
			...
			case SM_MAIN_MENU:
				...
				if(TCPIsPutReady(MySocket) >= 78u)
				{
					...
					// Position cursor at Line 13, Col 10
					TCPPutROMString(MySocket, (ROM BYTE*)"\x1b[5;1f");

                    TCPPutROMString(MySocket, strLocalIP);
					i = 0;
					while(1)
					{
						TCPPut(MySocket, (char)btohexa_high(AppConfig.MyIPAddr.v[i]));
	    				TCPPut(MySocket, btohexa_low(AppConfig.MyIPAddr.v[i]));
	    				if(++i == 4u)
	    				break;
	    				TCPPut(MySocket,'.');
					}
		
					TCPPutROMString(MySocket, strLEDs);
					TCPPut(MySocket, LED0_IO ? '1':'0');
					TCPPut(MySocket, ' ');
					TCPPut(MySocket, LED1_IO ? '1':'0');

					TCPPutROMString(MySocket, strAuxIO);
					TCPPut(MySocket, AUX0_IO ? '1':'0');
					TCPPut(MySocket, ' ');
					TCPPut(MySocket, AUX1_IO ? '1':'0');
					TCPPut(MySocket, ' ');
					TCPPut(MySocket, AUX2_IO ? '1':'0');
					TCPPut(MySocket, ' ');
					TCPPut(MySocket, AUX3_IO ? '1':'0');
					TCPPutROMString(MySocket, strBreak);
					TCPPutROMString(MySocket, strBreak);

					TCPPutROMString(MySocket, strMenu);
